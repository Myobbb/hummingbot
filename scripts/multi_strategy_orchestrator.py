...
class ArbitrageMInstanceConfig(BaseModel):
    """Configuration for a single arbitrage_m or arbitrage_l strategy instance"""
    name: str = Field(..., description="Unique name for this strategy instance")
    strategy_type: str = Field(default="m", description="Strategy type: 'm' for market orders, 'l' for limit orders")
...
class MultiStrategyOrchestrator(ScriptStrategyBase):
    ...
    def _add_arbitrage_m_strategy(self, config: ArbitrageMInstanceConfig, paused: bool = False):
        """
        Add an arbitrage_m or arbitrage_l strategy instance.

        This method:
        1. Builds market pairs from the shared connector pool
        2. Creates an ArbitrageMStrategy or ArbitrageLStrategy instance based on strategy_type
        ...
        """
        strategy_type = getattr(config, 'strategy_type', 'm')
        strategy_name = "arbitrage_l" if strategy_type == "l" else "arbitrage_m"
        self.logger().info(f"Adding {strategy_name} strategy: {config.name}")
        ...
        # Choose the correct MarketPair class based on strategy_type
        strategy_type = getattr(config, 'strategy_type', 'm')
        MarketPairClass = ArbitrageLMarketPair if strategy_type == "l" else ArbitrageMMarketPair
        StrategyClass = ArbitrageLStrategy if strategy_type == "l" else ArbitrageMStrategy
