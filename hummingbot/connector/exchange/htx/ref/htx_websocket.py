...
class HtxWebSocketHandler(BaseWebSocketHandler):
    ...
    async def _handle_message(self, message) -> None:
        ...
        try:
            ...
            # Robust heartbeat handling across HTX variants
            # Keep ONLY private v2 'action' ping handling active. Other variants are kept as reference.
            #
            # 1) Top-level market-style heartbeat: {"ping": <ts>} / {"pong": <ts>}
            # if isinstance(data, dict) and ("ping" in data or "pong" in data):
            #     try:
            #         if "ping" in data:
            #             ts_val = data.get("ping")
            #             await self._send_message({"pong": ts_val})
            #             try:
            ...
            #         pass
            # 2) "op":"ping" variant (seen in some channels)
            # try:
            #     if isinstance(data, dict) and str(data.get("op") or "").lower() == "ping":
            #         ts_val = data.get("ts") or (data.get("data") or {}).get("ts")
            #         await self._send_message({"op": "pong", **({"ts": ts_val} if ts_val is not None else {})})
            #         try:
            #             await self._handle_pong()
            #         except Exception:
            #             pass
            #         return
            #     if isinstance(data, dict) and str(data.get("op") or "").lower() == "pong":
            #         await self._handle_pong()
            #         return
            # except Exception:
            #     pass
